...
const kotsu = [
'https://media.giphy.com/media/WF1bJNFH8mE92/giphy.gif',
'https://ii.yuki.la/0/8a/8f669d5929c88367a2ef3fbb569a1544e0305708351878fac5e9f40b732e28a0.gif',
'https://i.gifer.com/8XFI.gif',
'https://cdn.pastemagazine.com/www/articles/Peridot%20marshmallows.gif',
'https://thumbs.gfycat.com/RequiredCaringGalapagossealion-small.gif',
];
...
client.on('guildMemberAdd', member => {
  ...
  member.guild.fetchInvites().then(guildInvites => {
    ...
    const logChannel = member.guild.channels.find(channel => channel.name === "bot-log");
    // A real basic message with the information we need.
    logChannel.send(`${member.user.tag} joined using invite code ${invite.code} from ${inviter.tag}. Invite was used ${invite.uses} times since its creation.`);
  });
  // Send the message to a designated channel on a server:
  const channel = member.guild.channels.find(ch => ch.name === 'user-join');
...
if (args[0]) message.channel.send(fortunes[Math.floor(Math.random() * fortunes.length)]);
  ...
  if(command === "brando") {
    return message.channel.send("https://tenor.com/view/brando-gif-5515978")
  }
  if(command === "birb") {
  ...
  if(command === "blub") {
    message.delete();
    return message.channel.send(":fish:** Blub Blub**:fish:")
  }
  if(command === "bailey") {
    message.channel.send("https://tenor.com/view/dance-gif-5025933")
  }
  if(command === "bylat") {
    return message.channel.send(`☭☭☭ SUKA BYLAT! ☭☭☭`)
  }
  if(command === "fish") {
  const request = require('request-promise-native');
    let options = {
...
let conf = JSON.parse(fs.readFileSync("./conf.json", "utf8"));
  ...
  if(command === "dead") {
    ...
    return message.channel.send(`${author} has died`)
  }
  if(command === "kitty") {
      const request = require('node-superfetch');
    const { body } = await request.get('https://aws.random.cat/meow');
return message.channel.send({ files: [body.file] });
  ...
  if(command === "kotsu") {
    message.channel.send({
        file: kotsu[Math.floor(Math.random() * kotsu.length)]
    });
  }
  if(command === "msbugaboo") {
    const request = require('node-superfetch');
  const { body } = await request.get('https://aws.random.cat/meow');
return message.channel.send({ files: [body.file] });}
...
snekfetch.get('https://api.chucknorris.io/jokes/random')
  ...
  if(command === "dance") {
    message.channel.send("https://tenor.com/view/dance-getit-gif-5363080")
  }
  if(command === "darth47") {
    return message.channel.send("https://tenor.com/view/heavy-breathing-cat-gif-5171487")
  }
  if(command === "dog") {
      const request = require('node-superfetch');
...
return message.channel.send({ files: [body.message] });
  ...
  if(command === "donthurtme") {
    message.channel.send("https://tenor.com/view/badlove-whatislove-head-bang-baby-dont-hurt-me-gif-11894011")
  }
  if(command === "hello") {
    return message.channel.send("https://tenor.com/view/penguin-hello-hi-heythere-cutie-gif-3950966")
  }
...
await message.channel.send({
  ...
  if(command === "goodbye") {
    return message.channel.send("https://tenor.com/view/goodbye-homer-gif-10101012")
  }
  if(command === "jumbo") {
  if (args.length < 1) {
  ...
  if(command === "kill") {
    ...
    let user = message.mentions.users.first();
    if (!user) {
    return message.channel.send("please mention someone that is killable")
  }
    let options = {
    url: 'http://api.giphy.com/v1/gifs/search',
    qs: {
  ...
  await message.channel.send({
    embed: {
      title: `${message.author.username} killed ${user.username}`,
      image: {
        url: response.data[Math.floor(Math.random() * response.data.length)].images.original.url
      },
      footer: {
...
message.channel.send({ embed });
  ...
  if(command === "live") {
    let author = message.author;
    return message.channel.send(`${author} LIIIIIIVES!`);
  }
  if(command === "marry") {
    const snekfetch = require("snekfetch")
...
request("https://reddit.com/r/meme/random.json", function (error, response, body) {
 ...
 message.channel.send({embed});
  ...
  if(command === "mmmm") {
    message.channel.send("https://tenor.com/view/iknow-gif-10535587")
  }
  if(command === "no") {
    const request = require('request-promise-native');
    let options = {
  ...
  if(command === "pleasekillme") {
  return message.channel.send("https://cdn.discordapp.com/attachments/472461572577427458/543490950278676480/4c974afccc77f0cec5d043cf596b8674.png")
}
  if(command === "poke") {
    const request = require('request-promise-native');
  let user = message.mentions.users.first();
if (!user) {
  return message.channel.send("please mention someone that can be poked")
}
  let options = {
url: 'http://api.giphy.com/v1/gifs/search',
qs: {
...
await message.channel.send({
embed: {
  title: `${message.author.username} poked ${user.username}`,
  image: {
    url: response.data[Math.floor(Math.random() * response.data.length)].images.original.url
  },
  footer: {
 ...
 .then(url => {
  ...
  if(command === "punch") {
    ...
    let user = message.mentions.users.first();
    if (!user) {
    return message.channel.send("please mention someone that is punchable")
  }
    let options = {
    url: 'http://api.giphy.com/v1/gifs/search',
    qs: {
  ...
  await message.channel.send({
    embed: {
      title: `${message.author.username} punched ${user.username}`,
      image: {
        url: response.data[Math.floor(Math.random() * response.data.length)].images.original.url
      },
      footer: {
...
return message.channel.send({ files: [body.message] });
  ...
  let user = message.mentions.users.first();
if (!user) {
  return message.channel.send("please mention someone that is reviveable")
}
  let options = {
url: 'http://api.giphy.com/v1/gifs/search',
qs: {
...
await message.channel.send({
embed: {
  title: `${message.author.username} revived ${user.username}`,
  image: {
    url: response.data[Math.floor(Math.random() * response.data.length)].images.original.url
  },
  footer: {
  ...
  if(command === "sham") {
    message.channel.send("https://tenor.com/view/lindsey-graham-sham-gif-12628822")
  }
  if(command === "ship") {
...
setTimeout(function(){
 message.channel.send(message.author.toString() + ` The Timer Has FINISHED!, it lasted: ${ms(ms(Timer), {long: true})}`)
  ...
  if(command === "trigger") {
    message.channel.send("https://tenor.com/view/ultra-instinct-goku-dbs-gif-11805676")}
  if(command === "wolfcomp") {
    message.channel.send("https://cdn.discordapp.com/attachments/506894253067075584/538929801633988608/image0.jpg")}
  if(command === "yes") {
    const request = require('request-promise-native');
    let options = {
    url: 'http://api.giphy.com/v1/gifs/search',
  ...
  let user = message.mentions.users.first();
  if (!user) {
  return message.channel.send("please mention someone that is yeetable")
  }
  let options = {
  url: 'http://api.giphy.com/v1/gifs/search',
  qs: {
  ...
  await message.channel.send({
  embed: {
  title: `${message.author.username} yeeted ${user.username}`,
  image: {
    url: response.data[Math.floor(Math.random() * response.data.length)].images.original.url
  },
  footer: {
  ...
  if(command === "links") {
          return message.channel.send(" **Links for Cosmo Craft**\n :shopping_cart: - https://shop.cosmo-craft.com/ (Buycraft) \n :microphone2: - https://discord.cosmo-craft.com/ (Discord)\n :game_die: - cosmo-craft.com (Minecraft)")
  }
  if(command === "rules") {

message.channel.send(
                                                "**__Official Cosmo Craft Rules__**\n" +
                                              "**1**. No `Un-wanted Behavior`, this means anything that is portrayed as aggressive or annoying towards someone else. The only time that is allowed is in the #spam channel, any other time is not allowed. If you are displaying a negative behavior, wherever it may be, you will be asked to leave for the moment to calm down. If you carry on in that current state, you will be muted for an hour as your first time punishment, a second time will result in a kick from the minecraft server and a ten minute cooldown before rejoining, a third time will be a temporary ban for a day, anything over the third offence will result in a 3 day additional ban, if you persist you will be permanently muted without a chance to appeal.\n")
message.channel.send(
                            "**2**. No `Advertising or disclosing information about other servers`. We hold pride to the name Cosmo Craft and don't want other people to tarnish the name. Anyone caught doing this will be muted for 3 days for the first offence. The second offence will be a ban from the minecraft server for one week. If you are caught a third time you will be permanently banned from bot the minecraft and discord server.\n"

)
message.channel.send(
                       "**3**. No `Unauthorized` modifications, this means something that gives you an unfair advantage compared to the other players (i.e `X-ray, Auto Clicker, Movement Tool`) if you are uncertain as to what falls under this rule, please contact moderators and above. Anyone caught with blacklisted mods will be banned for 1 week as a first offence. The second offence will lead to a permanent ban from both discord and the minecraft server."
)
message.channel.send(
                      "**4**. No `AFK` machines. This makes it very hard for the server to operate properly because of the constantly loaded environment around you while others are discovering new land. It also gives a major advantage to the player abusing the AFK machine. If you are uncertain about what an AFK machine is, contact staff and ask. Anyone that is caught with a machine that symbolizes AFK abuse will be punished. 7 day ban for the first offence. The second offence will be a month ban from the server. The third time that you are caught with one will result in a permanent ban from both the discord and minecraft server. Please be respectful of the rules. This makes everyone's lives better."
)
}
  if(command === "weather") {
 ...
 var mes = args.join(' ')
  ...
  message.author.send(
                          "**__Moderation Commands__: Access Trainee+**\n" +
                           ...
                           "`;" + "voiceboot [@NAME]` - removes the mentioned member from the voice channels\n" +
  ...
  message.author.send(
                           ...
                           //"`;" + "rules` - Shows the rules for Cosmo Craft!\n" +
                           "`;" + "bottime` - shows what time the bot is currently in\n" +
                           //"`;" + "links` - shows the links for Cosmo Craft!\n" +
                           "`;" + "weather [AREA CODE]` - shows the weather of the inputted location\n" +
                           ...
                           //"`;" + "cosmo-craft` - shows information about Cosmo Craft!\n" +
                           ...
                           "`;" + "help/commands` - show the main help page \n" +
  ...
  message.author.send(
                           ...
                           "`;" + "cat/kitty` - see some cats!\n" +
                           "`;" + "dog/pupper/puppy` - see some dogs!\n" +
                           "`;" + "meme(s)` - see some memes!\n" +
                           "\n" + "\n");
  message.author.send(
                           "**__Misc Commands__: Access All**\n" +
                           "`;" + "jumbo [EMOJI]` - super-size your emoji\n" +
                           "`;" + "roast [@NAME]` - roast someone\'s existance\n" +
                           "`;" + "punch [@NAME]` - punch someone in the face!\n" +
                           "`;" + "revive [@NAME]` - bro I got RAYGUN!\n" +
                          // "`;" + "kotsu` - thank you peridot\n" +
                           "`;" + "no` - DENIED\n" +
                           "`;" + "yes` - GRANTED\n" +
                           "`;" + "mmmm` - MMMMMMM\n" +
                           //"`;" + "wolfcomp` - dude runs off of a pizza box\n" +
                           //"`;" + "sham` - SHAMALAMA DING DONG\n" +
                           //"`;" + "bailey` - This one\'s for Bailey!\n" +
                           //"`;" + "msbugaboo` - MEOW MEW\n" +
                           "`;" + "cry` - show some tears\n" +
                           "`;" + "trigger` - GO SUPER SAIYAN\n" +
                           "`;" + "poke [@NAME]` - dang it's like Facebook!\n" +
                           "`;" + "lastmessage [@NAME]` - shows the last message of the mentioned user.\n" +
                           "`;" + "yeet [@NAME]` - YA YEEEEET!\n" +
                           "`;" + "kill [@NAME]` - go on, DO IT!\n" +
                           //"`;" + "bylat` - serj wanted this one\n" +
                           "`;" + "pleasekillme` - *craving of death increases*\n" +
                           //"`;" + "blub` - here you go Cyko\n" +
                           "`;" + "fish` - see some of da fishies!\n" +
                           "`;" + "bingo` - play some bingo with the bot\n" +
                           "`;" + "russ` - you feeling the lucky comrade?\n" +
                           "`;" + "calc` - allows you to put mathematical terms into the bot and it will give you back an answer\n" +
                           "`;" + "shoot [@NAME]` - *insert Stand-off music here*\n" +
                           "`;" + "avatar` - see you or the mentioned persons avatar \n" +
                           "`;" + "showerthoughts` - this will keep you up at night\n" +
                           "`;" + "marry [@NAME]` - marry someone that you love!\n"+
...
message.author.send(
                        "**__Moderation Commands__: Access Trainee+**\n" +
                         "`;" + "warn [@NAME] [REASON]` - warn the mentioned user (Trainee+)\n" +
                         "`;" + "mute [@NAME] (Duration) (Reason)` - mute a mentioned member (Mod+)\n" +
                         "`;" + "voiceboot [@NAME]` - removes the mentioned member from the voice channels (Mod+)\n" +
                         "`;" + "unmute [@NAME]` - unmutes the mentioned member (Mod+)\n" +
                         "`;" + "ban [@NAME] (Reason)` - ban the mentioned user (Mod+)\n" +
                         "`;" + "kick [@NAME]` - kick the mentioned user (Mod+) \n" +
                         "`;" + "purge [NUMBER]` - delete a certain number of messages (Mod+) \n" +
...
.addField(`Creators`,`The developers of my source code is <@339476546001698818> and <@196829450292297729>.`)
...
message.channel.send({
 ...
 .addField("Support Server", `You can join our support server [here](https://discord.gg/8txGPrB).`)
 .setTimestamp()
 .setColor("RANDOM")
 message.channel.send({embed: em});
  }
  ...
  if(command === "lastmessage") {
     ...
     let target = message.mentions.members.first() || message.guild.members.get(args[0]) || message.author;
  ...
  if(command === "bottime") {
return message.channel.send(`The current date and time for the bot is ${new Date()}`)
}
  if(command === "cosmo-craft") {
  return message.channel.send(`The official Minecraft Server **Cosmo Craft** was started in July of 2018 and is owned by <@196829450292297729>, <@339476546001698818>, and <@131949288065466368>. We are very happy to be a part of this wonderful community, if at any time you have any questions pm the staff or owners to get more information!`)
  }
  if(command === "avatar") {
...
.catch(() => console.error('Emoji failed to react.'));
 ...
 message.channel.send({embed: embed});
  ...
  if(command === "creators") {
return message.channel.send('The creators of this bot are <@339476546001698818> and <@196829450292297729>.')
}
  if(command === "myid") {
 message.channel.send(`Your ID is: ${message.author.id}`)
  ...
  if(command === "channelinfo") {
    ...
    if(!channel) return message.reply(":x: " + `| Please Mention a valid Channel!`);
    let embed = new Discord.RichEmbed()
  ...
  if(command === "roleinfo") {
    ...
    if(!role) return message.reply(":x: " + `| Please Mention a valid Role!`);
    let embed = new Discord.RichEmbed()
    .addField("Role Name:", `${role.name}`, true)
    .addField("Hex Color:", `${role.hexColor}`, true)
    .addField("Is Role Mentionable:", `${role.mentionable}`, true)
 ...
 if (message.channel.permissionsFor(message.author.id).has("MANAGE_CHANNELS") === false) {
   const embed =  new Discord.RichEmbed()
     .setColor(0xDD2E44)
     .setTimestamp()
     .setTitle(":x: ERROR: MISSING PERMISSIONS! :x:")
     .setDescription("You do not have the correct permissions for this command!");
   return message.channel.send({embed});
 }
let channel = message.mentions.channels.first();
if (!channel) {
return message.channel.send("Error! Please mention a valid channel to delete");
}
channel.delete()
...
if (!reason) {
  const Lockembed = new Discord.RichEmbed()
    ...
    .setTimestamp()
    .setTitle("🔒 LOCKDOWN NOTICE 🔒")
    .setDescription(`This channel has been put on lockdown by ${message.author.tag} for ${time}`);
}
 ...
 if (validUnlocks.includes(time)) {
  ...
  if(!message.member.roles.some(r=>["Administrator", "Moderator", "Trainee"].includes(r.name)) )
    return message.reply("Sorry, you don't have permissions to use this!");
  if (!logchannel) return message.reply('I cannot find a `#bot-log` channel. Please create one to log the warnings');
  ...
  message.channel.send(`:white_check_mark: Success! I\'ve logged the warning in ${logchannel.name}.\n`)
...
try {
    ...
    musicbot.executeQueue = (msg, queue) => {
      if (queue.songs.length <= 0) {
        msg.channel.send(musicbot.note('note', 'The music has ended and I have left the channel'));
      ...
      new Promise((resolve, reject) => {
          ...
          if (!video) {
            ...
            if (!video) {
              msg.channel.send(musicbot.note('note', 'I have finished playing and left the channel'));
              musicbot.emptyQueue(msg.guild.id);
              const voiceConnection = client.voiceConnections.find(val => val.channel.guild.id == msg.guild.id);
              if (voiceConnection !== null) return voiceConnection.disconnect();
            }
            ...
            dispatcher.on('end', () => {
              setTimeout(() => {
                ...
                if (musicbot.queues.get(msg.guild.id).songs.length > 0) {
                  ...
                  if (msg && msg.channel) msg.channel.send(musicbot.note('note', 'I have finished playing and left the channel.'));
                  musicbot.queues.set(msg.guild.id, {songs: [], last: null, loop: "none", id: msg.guild.id, volume: musicbot.defVolume});
                  if (musicbot.musicPresence) musicbot.updatePresence(musicbot.queues.get(msg.guild.id), msg.client, musicbot.clearPresence).catch((res) => { console.warn(`[MUSIC] Problem updating MusicPresence`); });
                  const voiceConnection = client.voiceConnections.find(val => val.channel.guild.id == msg.guild.id);
                  if (voiceConnection !== null) return voiceConnection.disconnect();
...
// Eval Command (ONLY FOR BRANDON, CAN BREAK SERVER)
const clean = text => {
